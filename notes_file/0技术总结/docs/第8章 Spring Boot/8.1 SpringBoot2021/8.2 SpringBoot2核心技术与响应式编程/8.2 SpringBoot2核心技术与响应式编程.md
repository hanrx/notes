  ...
  * 运行SpringApplication
    ...
    * 准备环境 prepareEnvironment();
      ...
      * 配置环境信息对象。
        * 读取所以的配置源的配置属性值。
      * 绑定环境信息
      * 监听器调用environmentPrepared；通知所有的监听器，当前环境准备完成。
    * 创建IOC容器（createApplicationContext()）
      * 根据项目类型（Servlet）创建容器。
      * 当前会创建 AnnotationConfigServletWebApplicationContext
    ...
    * 所有监听器 调用listeners.started(context);通知所有的监听器started  
    * 调用所有runnersh; callRunners()
    ...
    * 如果以上有异常，
      * 调用Listener的failed
    * 调用所有监听器的running方法，listeners-running(context);
