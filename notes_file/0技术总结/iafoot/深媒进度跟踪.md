...
## 20220322
- 问题清单
  - 图文没有 驳回 和审核中：添加字典类型【sys_b_publish_status】。@桑宁参照其它数据字典获取方式同步获取该数据字典内容。正式环境已同步更新。
    - （最简实现用【【sys_b_publish_status】】替换【【sys_publish_status】】需要测试）![](.深媒进度跟踪_images/07d6fd76.png)
  - 视频添加审核通过   驳回
    - 接口路径：studyVideo/editIsPublish。 传参参考编辑接口。需要传参【uid】和【发布状态：isPublish 1已发布、0草稿、2其他、3审核中  4已驳回】
    - ![](.深媒进度跟踪_images/5a7f71b2.png)
  - 用户查询  按照协会查询  注册时间查询（传的字段 和对应的值）
    - 参考：![](.深媒进度跟踪_images/68275ba6.png)
    - 用户查询按照 申请成为新闻负责人查询。查询条件添加参数【userTag  》 4】
      - ![](.深媒进度跟踪_images/1318fe26.png)
...
- 问题清单梳理：
  ...
  - 登录问题修改【少鹏解决】
  - 我的积分，相关查询接口。【已补充】
  - 我的积分存储相关接口：被点赞、被评论、被收藏。均存在每周上限。完成度40%【已补充】【差每周上限】【已补充】 》差分享
  - 总数：关注数【关注别人】、粉丝数【被关注的】、收藏【自己收藏的 图文+视频】、浏览数【图文+视频】【已补充】
  - 我的浏览数据【图文/视频】不对。牵扯面多。可以编辑和删除。【已补充】删除接口少鹏提供
  - 我的收藏【图文/视频】不对。牵扯面多。可以编辑和删除。【已补充】删除接口少鹏修改
  - 登录问题bug，验证码、秘密、Tocken的策略。【少鹏解决】
  - 消息中心：全部、粉丝、评论、点赞、分享等接口。》一个接口返回全部即可【已添加】
  - 协会积分接口添加。
  - APP 账户用户删除》 在后台管理界面 删除一个用户   删除成功了   你返回的结果也是成功的【已补充】
  - 搜索视频列表没有是否关注的自段【已处理】
  - 视频列表浏览数都是0。待确认
  - 删除用户，删除不掉。【已处理】
  
- 需要协助处理的问题
  - 意见反馈。反馈包含图片。包含后台管理界面【查看和删除】。
  - 图片/视频上云。

- 目前看后台缺失功能：
  - 视频审核功能 缺失：包含功能 发布、驳回。
  - 用户管理：缺少申请列表，审批用户成为【新闻负责人】- 
